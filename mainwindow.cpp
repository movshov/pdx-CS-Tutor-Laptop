...
MainWindow::MainWindow() {    
  ...
  connect(classWindow->cs299, SIGNAL(clicked()), this, SLOT(classCS299ButtonPressed()));
  ...
  connect(otherWindow->loginButton, SIGNAL(clicked()), this, SLOT(otherLogInButtonPressed()));
  connect(otherWindow->classDialog, SIGNAL(returnPressed()), this, SLOT(otherLogInButtonPressed()));
  connect(otherWindow->cancelButton, SIGNAL(clicked()), this, SLOT(otherCancelButtonPressed()));
...
void MainWindow::classCS299ButtonPressed() {
  klass = "CS299";
  classWindow->closeWindow();
  locationWindow->openWindow();
}
...
void MainWindow::classOtherButtonPressed() {
  ...
  // klass = otherWindow->classDialog->text();
  // locationWindow->openWindow();
}
...
void MainWindow::otherLogInButtonPressed() {
    //user may type "CS201" or "CS 201" either will work.
  if (otherWindow->classDialog->cursorPosition() < 5 || otherWindow->classDialog->cursorPosition() > 6) {
    otherWindow->closeWindow();
    otherWindow->openWindow();
    errorText->move(910, 525);
    errorText->setText("Please enter a class with at least 5 characters. EX: CS163 ");
    errorText->show();
    return;
  }
  klass = otherWindow->classDialog->text();
...
void MainWindow::otherCancelButtonPressed() {
  errorText->hide();
  otherWindow->closeWindow();
  theList->show();
  tutorsOnDutyList->show();
